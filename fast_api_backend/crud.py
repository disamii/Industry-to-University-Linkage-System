...
def get_users(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.StaffProfile).offset(skip).limit(limit).all()


def create_user(db: Session, user: schemas.UserCreate):
    role = user.role if hasattr(user, 'role') else models.UserRole.USER
    password_val = get_password_hash(
        user.password) if role == models.UserRole.ADMIN else user.password
...
def create_user_from_rpms(db: Session, rpms_user_data: dict):
    ...
    valid_fields = {
        column.name for column in models.StaffProfile.__table__.columns}
    filtered_data = {k: v for k, v in rpms_user_data.items()
...
def update_industry_profile(db: Session, industry_id: str, profile_update: schemas.IndustryProfileUpdate):
    db_industry = db.query(models.Industry).filter(
        models.Industry.id == industry_id).first()
    if not db_industry:
        return None
...
def create_org_unit(db: Session, name: str, unit_type: str, **kwargs):
    db_unit = models.OrganizationalUnit(
        name=name, unit_type=unit_type, **kwargs)
    db.add(db_unit)
    db.commit()
    db.refresh(db_unit)
    return db_unit
...
def create_industry_request(db: Session, industry_id: str, title: str, **kwargs):
    db_request = models.IndustryRequest(
        industry_id=industry_id, title=title, **kwargs)
    db.add(db_request)
    db.commit()
...
def get_industry_requests_by_industry(db: Session, industry_id: str):
