...
type Props = {
  academic_unit: OrgUnitResponse | string | null;
};

const OrgUnitCard = ({ academic_unit }: Props) => {
  if (!academic_unit) return null;

  const isString = typeof academic_unit === "string";
        ...
        <div className="flex justify-between items-center bg-muted/50 p-3 rounded-lg">
          <div>
            <h3 className="font-semibold text-base tracking-tight">
              {isString ? academic_unit : academic_unit?.name}
            </h3>
            {!isString && (
              <p className="text-xs">
                {academic_unit.total_subnodes} Sub Units
              </p>
            )}
          </div>
          {!isString && (
          ...
          <Collapsible defaultOpen>
            ...
            <CollapsibleContent className="relative ml-2 py-1 pl-4">
              <div className="space-y-0">
                {[...(academic_unit.ancestors || []), academic_unit].map(
                  (unit, index) => {
