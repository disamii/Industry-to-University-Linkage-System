...
def require_industry(current_user:  StaffProfile = Depends(get_current_active_user)):
    ...
    if current_user.role != UserRole.INDUSTRY:
        raise ForbiddenException(
            detail="Access denied. Industry role required")
    return current_user


def require_user(current_user:  StaffProfile = Depends(get_current_active_user)):
    """Dependency that requires user role"""
    if current_user.role != UserRole.USER:
        raise ForbiddenException(detail="Access denied. User role required")
    return current_user
