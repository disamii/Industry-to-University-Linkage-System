...
def create_org_unit(unit: schemas.OrgUnitCreate, db: Session = Depends(auth.get_db), current_user: models.User = Depends(auth.get_current_active_user)):
    # Permission check might be needed
    return crud.create_org_unit(db=db, name=unit.name, unit_type=unit.unit_type, **unit.model_dump(exclude={"name", "unit_type"}))
...
def read_subunits(unit_id: str, db: Session = Depends(auth.get_db)):
    # Fetch subnodes from the relationship
    db_unit = crud.get_org_unit(db, unit_id=unit_id)
    if db_unit is None:
        raise HTTPException(status_code=404, detail="Organizational Unit not found")
    return db_unit.subnodes
